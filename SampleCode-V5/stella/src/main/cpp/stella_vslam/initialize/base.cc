...
bool base::find_most_plausible_pose(const eigen_alloc_vector<Mat33_t>& init_rots, const eigen_alloc_vector<Vec3_t>& init_transes,
    ...
    if (*max_num_valid_pts_iter < min_num_valid_pts_) {
        spdlog::debug("reject if the number of valid points does not fulfill the threshold");
        return false;
    }
    ...
    if (1 < num_similars) {
        spdlog::debug("reject if most plausible hypothesis is unclear, num_similars {}", num_similars);
        return false;
    }

    // reject if the parallax is too small
    if (init_parallax.at(max_num_valid_index) > std::cos(parallax_deg_thr_ / 180.0 * M_PI)) {
        spdlog::debug("reject if the parallax is too small");
        return false;
    }
    ...
    if (num_triangulated_pts.at(max_num_valid_index) < min_num_triangulated_) {
        spdlog::debug("reject if the number of 3D points does not fulfill the threshold");
        return false;
    }
