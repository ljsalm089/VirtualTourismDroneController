...
bool initializer::try_initialize_for_monocular(data::frame& curr_frm) {
    ...
    if (num_matches < min_num_valid_pts_) {
        ...
        spdlog::debug("current frame {} doesn't have enough matches to the initial frame {}, matches: {}, expected: {}",
                      curr_frm.id_, init_frm_.id_, num_matches, min_num_valid_pts_);
        reset();
        return false;
    }
    spdlog::debug("current frame {} has enough matches to the initial frame {}, matches: {}", curr_frm.id_, init_frm_.id_, num_matches);
